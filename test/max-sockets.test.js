...
describe('fetch: maxSockets', function () {
  it('loads two one after the other', function () {
    ...
    return Bluebird.all([
      ...
      foo.fetch('/echo/5', { baseUrl: 'http://127.0.0.1:3066' })
        .json().then(urlWithDate),
      bar.fetch('/echo/bar').json().then(urlWithDate),
    ])
      .then(function (results) {
        assert.hasType(Array, results);
        assert.deepEqual(['1', '2', '3', '4', '5', 'bar'], results.map(getUrlPath));

        var times = results.map(function (result) { return result.time; });
